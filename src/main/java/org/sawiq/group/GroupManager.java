        ...
        if ("INV".equals(type)) {
            ...
            if (mc.player != null) {
                Text clickableId = Text.literal(gid).styled(style -> style
                        .withUnderline(true)
                        .withClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/cpgroup accept " + gid))
                        .withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, Text.translatable("text.coordinatesplus.group.invite_click_hover"))));
